...
- [ ] UX-042 OPEN — Shared Calculator Input Validation and Error Recovery Upgrade
  - Scope: improve representative reusable calculator families so invalid or incomplete input explains what must be fixed, preserves already-entered safe values, focuses the first actionable problem, and offers a clear recovery path without changing calculation rules. Consolidate related locale/accessibility coverage into this single user-visible task rather than splitting it into field-by-field regression microtasks.
