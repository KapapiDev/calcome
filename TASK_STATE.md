...
- [ ] UX-043 OPEN — Shared Calculator Stale-Result and Recalculation Clarity Upgrade
  - Scope: prevent an already-rendered result from appearing current after the user changes calculator inputs. For representative reusable calculator families, clearly mark results that need recalculation or otherwise keep result state synchronized without changing formulas, stored user data, routes, or policy values. Consolidate shared locale/accessibility coverage into this user-visible task.