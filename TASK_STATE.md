...
- [ ] UX-041 OPEN — Shared Calculator First-Run Guidance and Example Entry Upgrade
  - Scope: reduce blank-form friction across representative reusable calculator families with concise bilingual first-run guidance and safe example-entry affordances where architecture permits. Reuse shared patterns, avoid invented policy-sensitive values, preserve calculator-specific validation and calculation logic, and bundle all related accessibility/locale regression coverage into this single user-visible task rather than splitting it into microtasks.
