...
describe("CalculatorResultContext", () => {
  ...
  it("offers a bilingual browser-print summary with calculator identity and no persistence", () => {
    ...
    expect(summary).toHaveTextContent("Calculator: Compound Interest Calculator");
  ...
  it("blocks printing when the displayed result is stale", () => {
    ...
    render(
      <div>
        <CalculatorResultContext metrics={[{ label: "Result", value: "$100", featured: true }]} />
    ...
    expect(screen.queryByTestId("print-result-summary")).not.toBeInTheDocument();
    expect(screen.getByText(/Recalculate before printing this summary/)).toBeInTheDocument();
  });

  it("compares only the immediately previous calculated result in-session", () => {
    const { rerender } = render(
      <CalculatorResultContext metrics={[{ label: "결과", value: "100", featured: true }, { label: "총액", value: "1,000" }]} />,
    );
    expect(screen.queryByTestId("result-comparison")).not.toBeInTheDocument();

    rerender(
      <CalculatorResultContext metrics={[{ label: "결과", value: "120", featured: true }, { label: "총액", value: "1,200" }]} />,
    );
    expect(screen.getByTestId("result-comparison")).toHaveTextContent("직전 계산과 비교");
    expect(screen.getByTestId("result-comparison")).toHaveTextContent("직전: 100");
    expect(screen.getByTestId("result-comparison")).toHaveTextContent("현재: 120");
    expect(screen.getByTestId("result-comparison")).toHaveTextContent("증가 · 변화량: 20");

    rerender(
      <CalculatorResultContext metrics={[{ label: "결과", value: "150", featured: true }, { label: "총액", value: "1,500" }]} />,
    );
    expect(screen.getByTestId("result-comparison")).toHaveTextContent("직전: 120");
    expect(screen.getByTestId("result-comparison")).not.toHaveTextContent("직전: 100");
  });

  it("shows deterministic direction and formatted delta only for safely comparable values", () => {
    const { rerender } = render(
      <CalculatorResultContext metrics={[{ label: "Payment", value: "$1,200.50", featured: true }, { label: "Duration", value: "10 years 2 months" }, { label: "Rate", value: "3.5%" }]} />,
    );
    rerender(
      <CalculatorResultContext metrics={[{ label: "Payment", value: "$1,050.25", featured: true }, { label: "Duration", value: "9 years 11 months" }, { label: "Rate", value: "3.5%" }]} />,
    );
  ...
  it("keeps differently formatted units side by side instead of inferring a delta", () => {
    const { rerender } = render(
      <CalculatorResultContext metrics={[{ label: "Amount", value: "$100", featured: true }]} />,
    );
    rerender(
      <CalculatorResultContext metrics={[{ label: "Amount", value: "€120", featured: true }]} />,
    );
    const comparison = screen.getByTestId("result-comparison");
  ...
  it("localizes comparison guidance and clears ephemeral history when results reset", () => {
    const { rerender } = render(
      <CalculatorResultContext metrics={[{ label: "Result", value: "$100", featured: true }, { label: "Total", value: "$1,000" }]} />,
    );
    rerender(
      <CalculatorResultContext metrics={[{ label: "Result", value: "$125", featured: true }, { label: "Total", value: "$1,250" }]} />,
    );
    expect(screen.getByTestId("result-comparison")).toHaveTextContent("Compare with the previous calculation");
    expect(screen.getByTestId("result-comparison")).toHaveTextContent("does not store your inputs");
    expect(screen.getByTestId("result-comparison")).toHaveTextContent("Increased · Delta: $25");

    rerender(
      <CalculatorResultContext metrics={[{ label: "Result", value: "-", featured: true }, { label: "Total", value: "-" }]} />,
    );
    expect(screen.queryByTestId("result-comparison")).not.toBeInTheDocument();
  });
  ...
  it("captures privacy-safe in-session scenarios and compares them side by side", () => {
    const { rerender } = render(
      <CalculatorResultContext metrics={[{ label: "Payment", value: "$100", featured: true }, { label: "Total", value: "$1,000" }]} />,
    );

    fireEvent.click(screen.getByRole("button", { name: "Save current result as scenario" }));
    expect(screen.getByTestId("scenario-comparison")).toHaveTextContent("Scenario 1");
    expect(screen.getByTestId("scenario-comparison")).toHaveTextContent("Inputs, URLs, and browser storage are not used");

    rerender(
      <CalculatorResultContext metrics={[{ label: "Payment", value: "$125", featured: true }, { label: "Total", value: "$1,250" }]} />,
    );
  ...
  it("blocks stale or duplicate scenario capture and clears scenarios with reset", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Save current result as scenario" }));
    fireEvent.click(screen.getByRole("button", { name: "Save current result as scenario" }));
    expect(screen.getByText("This result is already saved as a scenario.")).toBeInTheDocument();

    rerender(
      <div>
        <CalculatorResultContext metrics={[{ label: "Result", value: "$120", featured: true }]} />
        <div data-testid="stale-result-notice">Inputs changed.</div>
      </div>,
    );
    fireEvent.click(screen.getByRole("button", { name: "Save current result as scenario" }));
    expect(screen.getByText(/Recalculate before saving this scenario/)).toBeInTheDocument();

    rerender(
      <div>
        <CalculatorResultContext metrics={[{ label: "Result", value: "-", featured: true }]} />
      </div>,
    );
