...
import { DiscountSalePriceCalculator } from "./discount-sale-price-calculator";

export function LocalizedDiscountSalePricePage({
  locale,
}: {
  ...
  return (
    <main id="main-content" className="flex-1">
      ...
      <div className="mx-auto w-full max-w-[1440px] px-5 py-8 sm:px-6 sm:py-10">
        ...
        <div className="mt-6">
          <DiscountSalePriceCalculator locale={locale} />
        </div>
        <section className="mt-10 rounded-xl border bg-card p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-sm">
            <Link
              ...
              href={`/${locale}/finance/percentage`}
            >
              {locale === "ko" ? "퍼센트 계산기" : "Percentage Calculator"}
            </Link>
            <Link
              className="underline underline-offset-4"
              href={`/${locale}/finance/inflation-purchasing-power`}
            >
              {locale === "ko"
                ? "물가상승·구매력 계산기"
