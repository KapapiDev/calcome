...
describe("LtvCalculator", () => {
  it("clears a previous result and links validation errors after invalid resubmission", async () => {
    ...
    expect(screen.queryByText("계산하면 예상 LTV와 목표 비율 기준 대출 여력이 표시됩니다.")).not.toBeInTheDocument();

    await user.clear(propertyValue);
    await user.click(screen.getByRole("button", { name: "LTV 계산하기" }));

    expect(screen.getByRole("alert")).toHaveAttribute("id", "ltv-error-summary");
    ...
    expect(
      screen.getByText("계산하면 예상 LTV와 목표 비율 기준 대출 여력이 표시됩니다."),
    ).toBeInTheDocument();
  });
});
