...
export function LocalizedCurrencyConversionPage({ locale }: { locale: CurrencyConversionLocale }) {
  const ko = locale === "ko";
    ...
    <main id="main-content" className="flex-1">
      ...
      <div className="mx-auto w-full max-w-[1440px] px-5 py-8 sm:px-6 sm:py-10">
        <nav aria-label={ko ? "경로" : "Breadcrumb"} className="text-sm text-muted-foreground">
          <Link href={`/${locale}`}>{home}</Link> / <span aria-current="page">{title}</span>
        </nav>
        ...
        <section className="mt-10 rounded-xl border bg-card p-5">
          <h2 className="text-xl font-semibold">{ko ? "사용 방법" : "How to use it"}</h2>
          ...
          <h2 className="mt-6 text-xl font-semibold">{ko ? "주의할 점" : "What to consider"}</h2>
          <p className="mt-3 text-sm leading-7 text-muted-foreground">
            {ko
