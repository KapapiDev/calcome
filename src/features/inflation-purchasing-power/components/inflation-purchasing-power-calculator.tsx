...
import {
  calculateInflationPurchasingPower,
  type InflationPurchasingPowerResult,
} from "../calculate";
import {
...
import {
  validateInflationPurchasingPower,
  type InflationPurchasingPowerErrors,
  type InflationPurchasingPowerValues,
} from "../validation";
...
const initialValues: InflationPurchasingPowerValues = {
...
export function InflationPurchasingPowerCalculator({
  ...
  const [errors, setErrors] = useState<InflationPurchasingPowerErrors>({});
  const [result, setResult] = useState<InflationPurchasingPowerResult>();
  ...
  function submit(event: FormEvent) {
    ...
    const checked = validateInflationPurchasingPower(values, locale);
    ...
    requestResultScroll();
    setResult(calculateInflationPurchasingPower(checked.data));
  }
  ...
  return (
    <section aria-labelledby="inflation-purchasing-power-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <InputField
            ...
            onChange={(value) =>
              setValues((current) => ({
                ...current,
                currentAmount: formatMoneyInput(value, current.currentAmount),
              }))
            }
          />
          <InputField
            id="annualInflationPercent"
            label={copy.annualInflationPercent}
            value={values.annualInflationPercent}
        ...
        >
          ...
          <PrimaryResults
            metrics={[
              {
                label: copy.futurePurchasingPower,
                value: money(result?.futurePurchasingPower),
                featured: true,
              },
              {
                label: copy.purchasingPowerLoss,
                value: money(result?.purchasingPowerLoss),
              },
              {
                label: copy.purchasingPowerLossPercent,
                value: percent(result?.purchasingPowerLossPercent),
              },
              {
                label: copy.futureNominalCost,
                value: money(result?.futureNominalCost),
              },
              {
                label: copy.cumulativeInflation,
                value: percent(result?.cumulativeInflationPercent),
              },
            ]}
          />
