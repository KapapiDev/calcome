...
export function ParentalLeaveBenefitCalculator({
  ...
  function submit(event: FormEvent) {
    ...
    requestResultScroll();
    setResult(
      calculateParentalLeaveBenefit({ monthlyOrdinaryWage, leaveMonths }),
    );
  }
    ...
    <section aria-labelledby="parental-leave-benefit-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <h2
            id="parental-leave-benefit-input-title"
          ...
          <div className="mt-4">
            <label htmlFor="leaveMonths" className="block text-sm font-medium">
              {copy.leaveMonths}
            </label>
            <input
              id="leaveMonths"
              ...
              max="18"
              step="1"
              value={values.leaveMonths}
              onChange={(event) =>
                setValues((current) => ({
                  ...current,
                  leaveMonths: event.target.value,
                }))
              }
              aria-invalid={hasError}
              className={fieldClass}
        ...
        <section
          ref={resultRef}
          aria-labelledby="parental-leave-benefit-result-title"
          className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
        >
          <h2
            id="parental-leave-benefit-result-title"
            ...
            metrics={[
              {
                ...
                value: money(result?.totalBenefit),
                featured: true,
              },
              {
                label: copy.averageMonthly,
                value: money(result?.averageMonthlyBenefit),
              },
              {
                label: copy.firstThree,
                value: money(result?.firstThreeMonthly),
              },
              {
                label: copy.monthsFourToSix,
                value: money(result?.monthsFourToSixMonthly),
              },
              {
                label: copy.monthSevenPlus,
                value: money(result?.monthSevenPlusMonthly),
              },
            ]}
          />
