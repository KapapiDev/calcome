...
function initialValues(locale: OperatingProfitLocale) {
  return locale === "ko"
    ? {
        revenue: "100,000,000",
        costOfGoodsSold: "55,000,000",
        operatingExpenses: "25,000,000",
      }
    : {
        revenue: "100,000",
        costOfGoodsSold: "55,000",
        operatingExpenses: "25,000",
      };
}

export function OperatingProfitCalculator({
  locale,
}: {
  ...
  const [result, setResult] = useState<OperatingProfitResult>();
  const {
    resultRef,
  ...
  function submit(event: FormEvent) {
    ...
    try {
      const next = calculateOperatingProfit({
        revenue: parseNumber(values.revenue),
        costOfGoodsSold: parseNumber(values.costOfGoodsSold),
        operatingExpenses: parseNumber(values.operatingExpenses),
      });
      setHasError(false);
      requestResultScroll();
      setResult(next);
  ...
  return (
    <section aria-labelledby="operating-profit-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <h2
            id="operating-profit-input-title"
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {field("revenue", copy.revenue)}
            {field("costOfGoodsSold", copy.costOfGoodsSold)}
            {field("operatingExpenses", copy.operatingExpenses)}
          </div>
        ...
        <section
          ref={resultRef}
          aria-labelledby="operating-profit-result-title"
          className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
        >
          <h2
            id="operating-profit-result-title"
            ...
            metrics={[
              {
                ...
                value: number(result?.operatingProfit),
                featured: true,
              },
              {
                label: copy.operatingMargin,
                value: percent(result?.operatingMargin),
                featured: true,
              },
              {
                label: copy.grossProfit,
                value: number(result?.grossProfit),
              },
            ]}
          />
