...
export function LocalizedAgePage({ locale }: { locale: AgeLocale }) {
  const copy = ageContent[locale];
  const path = `/${locale}/finance/age`;
  ...
  return (
    <main id="main-content" className="flex-1">
      ...
      <div className="mx-auto w-full max-w-[1440px] px-5 py-8 sm:px-6 sm:py-10">
        ...
        <div className="mt-6"><AgeCalculator locale={locale} /></div>
        <section className="mt-10 rounded-xl border bg-card p-5">
          ...
          <h2 className="mt-6 text-xl font-semibold">{locale === "ko" ? "사용할 때 확인할 점" : "What to check"}</h2>
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-sm">
            <Link className="underline underline-offset-4" href={`/${locale}/finance/percentage`}>
              {locale === "ko" ? "퍼센트 계산기" : "Percentage Calculator"}
            </Link>
          </div>
        </section>
      </div>
