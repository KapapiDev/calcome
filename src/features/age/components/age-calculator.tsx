    ...
    <section aria-labelledby="age-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <h2 id="age-input-title" className="mt-1 text-xl font-semibold">
            {copy.input}
          </h2>
          {hasError ? (
            <p
        ...
        <section
          ref={resultRef}
          aria-labelledby="age-result-title"
          className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
        >
          <h2 id="age-result-title" className="text-xl font-semibold">
            {copy.result}
          </h2>
          <PrimaryResults
            metrics={[
              {
                label: copy.fullYears,
                value: result
                  ? `${numberFormatter.format(result.fullYears)} ${copy.yearsUnit}`
                  : "—",
                featured: true,
              },
              {
                ...
                value: result
                  ? `${numberFormatter.format(result.totalDays)} ${copy.daysUnit}`
                  : "—",
                featured: true,
              },
              {
                label: copy.weeks,
                value: result
                  ? `${numberFormatter.format(result.fullWeeks)} + ${numberFormatter.format(result.extraDays)} ${copy.daysUnit}`
                  : "—",
              },
              { label: copy.nextBirthday, value: result?.nextBirthday ?? "—" },
              {
                label: copy.daysUntilBirthday,
                value: result
                  ? `${numberFormatter.format(result.daysUntilBirthday)} ${copy.daysUnit}`
                  : "—",
              },
            ]}
          />
