...
import { calculateBondYield, type BondYieldResult } from "../calculate";
...
export function BondYieldCalculator({ locale }: { locale: BondYieldLocale }) {
  const copy = bondYieldContent[locale];
  const [values, setValues] = useState(initialValues);
  const [error, setError] = useState(false);
  const [result, setResult] = useState<BondYieldResult>();
  ...
  function submit(event: FormEvent) {
    ...
    const marketPrice = parse(values.marketPrice);
  ...
  return (
    <section aria-labelledby="bond-yield-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        <section
          ref={resultRef}
          aria-labelledby="bond-yield-result-title"
          className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
        >
          <h2 id="bond-yield-result-title" className="text-xl font-semibold">
            {copy.result}
          </h2>
          <PrimaryResults
            metrics={[
              {
                label: copy.ytm,
                value: result ? `${result.ytmPercent.toFixed(3)}%` : "—",
                featured: true,
              },
              {
                label: copy.currentYield,
                value: result
                  ? `${result.currentYieldPercent.toFixed(3)}%`
                  : "—",
              },
              {
                label: copy.annualCoupon,
                value: money(result?.annualCouponAmount),
              },
              {
                label: copy.totalCashFlow,
                value: money(result?.totalNominalCashFlow),
              },
            ]}
          />
