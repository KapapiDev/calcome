...
function initialValues(locale: RentalYieldLocale) {
  return locale === "ko"
    ? { purchasePrice: "500,000,000", monthlyRent: "2,000,000", annualOtherIncome: "0", annualOperatingCosts: "4,000,000", vacancyRatePercent: "5" }
    : { purchasePrice: "400,000", monthlyRent: "2,500", annualOtherIncome: "0", annualOperatingCosts: "6,000", vacancyRatePercent: "5" };
}

export function RentalYieldCalculator({ locale }: { locale: RentalYieldLocale }) {
  ...
  const percent = (value?: number) => value === undefined ? "—" : new Intl.NumberFormat(locale === "ko" ? "ko-KR" : "en-US", { maximumFractionDigits: 2 }).format(value) + "%";
  ...
  const setMoney = (key: "purchasePrice" | "monthlyRent" | "annualOtherIncome" | "annualOperatingCosts", value: string) => setValues((current) => ({ ...current, [key]: formatMoneyInput(value, current[key]) }));
  const moneyFields = [["purchasePrice", copy.purchasePrice], ["monthlyRent", copy.monthlyRent], ["annualOtherIncome", copy.annualOtherIncome], ["annualOperatingCosts", copy.annualOperatingCosts]] as const;
    ...
    <section aria-labelledby="rental-yield-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        <form onSubmit={submit} onFocusCapture={noteNumericInputFocus} noValidate className={`${compactCalculatorSettingsClass} min-w-0`}>
          ...
          <h2 id="rental-yield-input-title" className="mt-1 text-xl font-semibold">{copy.input}</h2>
          ...
          <div className="mt-4">
            <label htmlFor="vacancyRatePercent" className="block text-sm font-medium">{copy.vacancyRatePercent}</label>
            <input id="vacancyRatePercent" inputMode="decimal" value={values.vacancyRatePercent} onChange={(event) => setValues((current) => ({ ...current, vacancyRatePercent: event.target.value }))} aria-invalid={hasError} className={fieldClass} />
          </div>
        ...
        <section ref={resultRef} aria-labelledby="rental-yield-result-title" className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm">
          <h2 id="rental-yield-result-title" className="text-xl font-semibold">{copy.result}</h2>
          <PrimaryResults metrics={[
            { label: copy.netYieldPercent, value: percent(result?.netYieldPercent), featured: true },
            { label: copy.netOperatingIncome, value: money(result?.netOperatingIncome), featured: true },
            { label: copy.grossYieldPercent, value: percent(result?.grossYieldPercent) },
            { label: copy.grossAnnualIncome, value: money(result?.grossAnnualIncome) },
            { label: copy.effectiveAnnualIncome, value: money(result?.effectiveAnnualIncome) },
          ]} />
