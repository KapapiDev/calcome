...
import { AprApyCalculator } from "./apr-apy-calculator";

export function LocalizedAprApyPage({ locale }: { locale: AprApyLocale }) {
  ...
  return (
    <main id="main-content" className="flex-1">
      ...
      <div className="mx-auto w-full max-w-[1440px] px-5 py-8 sm:px-6 sm:py-10">
        ...
        <div className="mt-6"><AprApyCalculator locale={locale} /></div>
        <section className="mt-10 rounded-xl border bg-card p-5">
          ...
          <h2 className="mt-6 text-xl font-semibold">{locale === "ko" ? "계산 예시" : "Worked example"}</h2>
          <p className="mt-3 text-sm leading-7 text-muted-foreground">{copy.example}</p>
