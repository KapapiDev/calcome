...
function initialValues(locale: CryptoAverageCostLocale) {
  return locale === "ko"
    ? {
        currentQuantity: "2",
        currentAveragePrice: "50,000,000",
        additionalQuantity: "1",
        additionalPrice: "35,000,000",
      }
    : {
        currentQuantity: "2",
...
export function CryptoAverageCostCalculator({
  locale,
}: {
  ...
  const moneyField = (
    id: "currentAveragePrice" | "additionalPrice",
    label: string,
  ) => (
    <div>
    ...
    <section aria-labelledby="crypto-average-cost-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <h2
            id="crypto-average-cost-input-title"
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <div>
              <label
                ...
                className="block text-sm font-medium"
              >
                {copy.additionalQuantity}
              </label>
              <input
                id="additionalQuantity"
                inputMode="decimal"
                value={values.additionalQuantity}
                onChange={(event) =>
                  setValues((current) => ({
                    ...current,
                    additionalQuantity: event.target.value,
                  }))
                }
                aria-invalid={hasError}
                className={fieldClass}
        ...
        <section
          ref={resultRef}
          aria-labelledby="crypto-average-cost-result-title"
          className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
        >
          <h2
            id="crypto-average-cost-result-title"
          ...
          <PrimaryResults
            metrics={[
              {
                label: copy.newAveragePrice,
                value: money(result?.newAveragePrice),
                featured: true,
              },
              {
              ...
              {
                label: copy.totalQuantity,
                value: number(result?.totalQuantity),
              },
              {
                label: copy.additionalCost,
                value: money(result?.additionalCost),
              },
              {
                label: copy.averagePriceChangePercent,
                value: percent(result?.averagePriceChangePercent),
              },
            ]}
          />
