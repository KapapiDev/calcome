...
export function EmergencyFundCalculator({
  ...
  function updateMoney(
    key: "monthlyEssentialExpenses" | "currentSavings" | "monthlyContribution",
    value: string,
  ) {
    setValues((current) => ({
      ...current,
  ...
  const coverage = result
    ? `${result.currentCoverageMonths.toFixed(1)} ${locale === "ko" ? "개월" : "months"}`
    : "—";
  const monthsToGoal = result
    ? result.monthsToGoal === null
      ? locale === "ko"
        ? "월 저축액 입력 필요"
        : "Add a monthly contribution"
      : `${result.monthsToGoal} ${locale === "ko" ? "개월" : "months"}`
    : "—";
      ...
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        >
          ...
          <Field
            ...
            value={values.monthlyEssentialExpenses}
            suffix={currency}
            onChange={(value) => updateMoney("monthlyEssentialExpenses", value)}
          />
          <Field
            id="targetMonths"
            label={copy.targetMonths}
            value={values.targetMonths}
            suffix={locale === "ko" ? "개월" : "months"}
            onChange={(value) =>
              setValues((current) => ({ ...current, targetMonths: value }))
            }
          />
          <Field
            id="currentSavings"
            label={copy.currentSavings}
        ...
        <section
          ref={resultRef}
          aria-labelledby="emergency-fund-result-title"
          className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
        >
          <h2
            id="emergency-fund-result-title"
