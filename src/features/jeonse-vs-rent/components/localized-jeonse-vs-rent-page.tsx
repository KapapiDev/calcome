...
export function LocalizedJeonseVsRentPage({
  locale,
}: {
  ...
  return (
    <main id="main-content" className="flex-1">
      ...
      <div className="mx-auto w-full max-w-[1440px] px-5 py-8 sm:px-6 sm:py-10">
        ...
        <div className="mt-6">
          <JeonseVsRentCalculator locale={locale} />
        </div>
        <section className="mt-10 rounded-xl border bg-card p-5">
          ...
          <h2 className="mt-6 text-xl font-semibold">
            {locale === "ko" ? "비교할 때 주의할 점" : "What to consider"}
          </h2>
          <p className="mt-3 text-sm leading-7 text-muted-foreground">
            {copy.tips}
          </p>
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-sm">
            <Link
              ...
              href={`/${locale}/finance/jeonse-loan-limit`}
            >
              {locale === "ko"
                ? "전세대출 한도 계산기"
                : "Jeonse Loan Limit Calculator"}
            </Link>
            <Link
              className="underline underline-offset-4"
