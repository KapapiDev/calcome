...
export function JeonseVsRentCalculator({
  locale,
}: {
  ...
  return (
    <section aria-labelledby="jeonse-vs-rent-input-title">
      <div className={dashboardCalculatorWorkspaceClass}>
        ...
        <section
          ref={resultRef}
          aria-labelledby="jeonse-vs-rent-result-title"
          className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm"
        >
          <h2
            id="jeonse-vs-rent-result-title"
          ...
          <PrimaryResults
            metrics={[
              {
                label: copy.jeonseCost,
                value: money(result?.jeonseEconomicCost),
                featured: true,
              },
              {
                label: copy.rentCost,
                value: money(result?.rentEconomicCost),
                featured: true,
              },
              { label: copy.difference, value: money(result?.costDifference) },
              { label: copy.cheaperOption, value: cheaper },
              {
                label: copy.breakEvenRent,
                value: money(result?.breakEvenMonthlyRent),
              },
              {
                label: copy.jeonseInterest,
                value: money(result?.jeonseLoanInterest),
              },
              {
                label: copy.jeonseOpportunity,
                value: money(result?.jeonseOpportunityCost),
              },
              { label: copy.rentPayments, value: money(result?.rentPayments) },
              {
                label: copy.rentDepositOpportunity,
                value: money(result?.rentDepositOpportunityCost),
              },
            ]}
          />
