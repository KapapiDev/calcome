...
export const totalCompensationComparisonCalculator = {
  ...
  name: "오퍼 총보상 비교 계산기",
  description: "두 채용 제안의 연봉, 보너스, 주식보상, 복리후생과 사이닝 보너스를 같은 기간으로 환산해 비교합니다.",
  ...
  category.id === "employment"
    ...
    : category.id === "business-life"
      ...
      : category.id === "loan"
        ...
        : category.id === "housing"
          ? { ...category, calculatorIds: [...category.calculatorIds, "rent-affordability", "jeonse-vs-rent", "home-purchase-total-cost", "home-sale-net-proceeds", "rental-yield"] }
          : category,
...
const calculatorsById = new Map<string, PublishedCalculator>(allPublishedCalculators.map((calculator) => [calculator.id, calculator] as const));
...
export const visibleCalculatorDirectory = groupedCalculatorDirectory.filter((category) => category.calculators.length > 0);
